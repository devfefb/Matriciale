...
export class CalculosService {
   ...
   */
  async executarCalculos(municipioId: string): Promise<any> {
    try {
      console.log('🚀 Iniciando atualização dos campos calculados no Firestore via Service...');
        ...
        for (const medicamentoDoc of medicamentosSnapshot.docs) {
          ...
          try {
            ...
            const dadosParaSalvar = {
              ...
              medianas: {
                ...Object.entries(camposCalculados.medianas).reduce((acc: any, [key, value]) => {
                  acc[key] = value;
                  return acc;
                }, {})
              },
              ...
              contagens: {
                ...Object.entries(camposCalculados.contagens).reduce((acc: any, [key, value]) => {
                  acc[key] = value;
                  return acc;
                }, {})
              },
