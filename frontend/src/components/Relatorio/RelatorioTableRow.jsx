...
const RelatorioTableRow = ({ relatorios }) => {
    return (
        <tr className={styles.row}>
            ...
            <td><div className={styles.item}>{relatorios.metodo ? relatorios.metodo.toLocaleString('pt-BR') : '0'}</div></td>
            <td><div className={styles.item}>{relatorios.met_est ? relatorios.met_est.toLocaleString('pt-BR') : '0'}</div></td>
            <td><div className={styles.item}>{relatorios.estoque ? relatorios.estoque.toLocaleString('pt-BR') : '0'}</div></td>
            <td><div className={styles.item}>{relatorios.reposicao ? relatorios.reposicao.toLocaleString('pt-BR') : '0'}</div></td>
        </tr>
    )
}
